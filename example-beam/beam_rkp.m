...
M=round(400*1);
m=M/2;
yr=linspace(-2*y0,2*y0,M+1);
h=yr(2)-yr(1);
y=linspace(-2*y0-h,2*y0+h,M+3)+h/2;
y(end)=[];
yn=linspace(-2*y0,2*y0,m+1);

p=zeros(M+2,N+1);
pi=normpdf(yn(1:end),0,0.0025)';
P0=normcdf(yn(1:end),0,0.0025);
parfor i = 1:n
    p0=Pq(i)*pi/(P0(end)-P0(1));
    pyt=rkp(dt,h,N+1,M+2,m+1,y,yn,p0,ydot(i,:));
    p=p+pyt;
    i
end
...
yrkp=y;
prkp=p;
save beam_exc_rkp.mat trkp yrkp prkp